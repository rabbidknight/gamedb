...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetRandomHighRatedGames`(IN UserID INT)
BEGIN
    -- Temporarily store the list of genre IDs associated with the user's lists
    CREATE TEMPORARY TABLE UserGenres (GenreID INT);

    INSERT INTO UserGenres (GenreID)
    SELECT DISTINCT gg.GenreID
    FROM game_genres gg
    JOIN userlists ul ON ul.GameID = gg.GameID
    WHERE ul.UserID = UserID;

    -- Select games that have a rating higher than 5 and at least one matching genre with the user's preferred genres
    SELECT g.GameID, g.Name, g.ConsoleID, g.ReleaseYear, g.DeveloperID, g.List, g.Image, g.Rating
    FROM games g
    JOIN game_genres gg ON gg.GameID = g.GameID
    WHERE g.Rating > 5
    AND gg.GenreID IN (SELECT GenreID FROM UserGenres)
    GROUP BY g.GameID, g.Name, g.ConsoleID, g.ReleaseYear, g.DeveloperID, g.List, g.Image, g.Rating
    ORDER BY RAND()
    LIMIT 5;

    -- Drop the temporary table
    DROP TEMPORARY TABLE UserGenres;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
